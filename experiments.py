...
class InversePolynomial:
    ...
    def __call__(self, x):
        return 1 / self.polynomial(x)

    def poles(self):
        return self.polynomial.roots()

    def degree_denom(self):
        return self.polynomial.degree()


class InverseMonomial(InversePolynomial):
    def __init__(self, deg):
        super().__init__(np.polynomial.Polynomial.fromroots([0] * deg))
...
class ExpRationalApprox:
    ...
    def poles(self):
        return flamp.to_mp([-self.degree])


def fact1(problem, k, max_iter, n_grid, tol):
    return 2 * problem.fov_optimal_error_remez(
        k, max_iter=max_iter, n_grid=n_grid, tol=tol
    )
...
def thm1(problem, k):
    ...
    assert len(low_poles) + len(high_poles) == len(
        poles
    ), "Poles in the range of the eigenvalues are not allowed!"
    kappa_part = np.prod((lmax - low_poles) / (lmin - low_poles)) * np.prod(
        (high_poles - lmin) / (high_poles - lmax)
    )

    def C_fun(self, _):
        return self.f.degree_denom() * kappa_part

    def k_fun(self, k):
        return k - self.f.degree_denom() + 1
...
def thm2(problem, k, max_iter, tol):
    def C_fun(self, k):
        return 3 / np.sqrt(np.pi * k) * self.kappa()

    def k_fun(_, k):
        return k // 2

    if k >= 2:
        return problem.adjuster(
            "spectrum_optimal_error", C_fun, k_fun, k, max_iter=max_iter, tol=tol
        )
    else:
        return np.inf
...
def thm3(problem, k, max_iter, tol):
    ...
    if k >= 2:
        return problem.adjuster(
            "pseudo_spectrum_optimal_error", C_fun, k_fun, k, max_iter=max_iter, tol=tol
        )
    else:
        return np.inf
...
def worst_b0(f, spectrum, ks, bounds, norm_matrix_sqrt=None, xatol=1e-10):
    ...
    def objective(b0):
        ...
        lan = np.array(
            [problem.lanczos_error(k, norm_matrix_sqrt=norm_matrix_sqrt) for k in ks]
        )
        opt = np.array(
            [
                problem.instance_optimal_error(k, norm_matrix_sqrt=norm_matrix_sqrt)
                for k in ks
            ]
        )
...
def plot_convergence_curves(error_df, relative_error=True, **kwargs):
    ...
    ax.set(title=title, xlabel=k_label, ylabel=error_label, yscale="log")
    ax.legend(title="")
    handles, labels = ax.get_legend_handles_labels()
    # Undo the reversal above; lines should appear in
    # the legend in the order given by user.
