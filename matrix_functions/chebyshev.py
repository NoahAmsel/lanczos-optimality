...
def cheb_nodes(degree, a=-1., b=1., dtype=float):
    if dtype == np.dtype('O'):
        ...
        return (a+b)/2 + ((b-a)/2) * flamp.cos(
            linspace(
                1, 2 * degree + 1, num=(degree + 1), dtype=np.dtype('O')) * flamp.gmpy2.const_pi() / (2 * degree + 2))
    else:
...
def cheb_interpolation(degree, f, a, b, dtype=float):
    ...
    y = f(x)
    if dtype == np.dtype('O'):
        w = flamp.sin(linspace(1, 2 * degree + 1, num=(degree + 1),
                      dtype=np.dtype('O')) * flamp.gmpy2.const_pi() / (2 * degree + 2))
    else:
        w = np.sin(np.linspace(1, 2 * degree + 1, num=(degree + 1))
                   * np.pi / (2 * degree + 2))
