...
def test_discrete_remez_error():
    ...
    for degree in range(10):
        remez_error_continuous = mf.remez_error(degree, inv_sqrt, domain=(a, b), max_iter=10, tol=1e-12)
        remez_error_discrete = mf.discrete_remez_error(degree, inv_sqrt(points), points, max_iter=10, tol=1e-12)
...
def test_oscillations():
    ...
    remez_error_discrete = mf.discrete_remez_error(0, flamp.cos(points), points, max_iter=10, tol=1e-12)
    ...
    remez_error_discrete = mf.discrete_remez_error(0, vals, points, max_iter=10, tol=1e-12)
    assert np.abs(remez_error_discrete - 1) < flamp.gmpy2.mpfr('1e-11')
